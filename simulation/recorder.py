...
class video_recorder:
    """
    The class is designed to capture camera images as a single video
    """

    def __init__(self, view1_size, view2_size, path, fps):
...
class video_recorder_one_cam:
    """
        The class is desgined to capture one camera angle as a single video
        """

    def __init__(self, view1_size, path, fps):
        self.img_size = [view1_size[0], view1_size[1]]
        # print(self.img_size, vision_size, tactile_size)
        dir = os.path.dirname(os.path.abspath(path))
        os.makedirs(dir, exist_ok=True)
        if path.endswith('.avi'):
    ...
    def capture(self, view1_image):
        img_cap = self._align_image(view1_image)
        self.rec.write(img_cap)

    def _align_image(self, img1):
        assert img1.dtype == np.uint8
        assert img1.shape[0] == self.img_size[0]
        assert img1.shape[1] == self.img_size[1]

        new_img = np.zeros([self.img_size[0], self.img_size[1], 3], dtype=np.uint8)
        new_img[:img1.shape[0], :img1.shape[1]] = (img1[..., :3])[..., ::-1]
        return new_img
