...
float max_mot_pos = 0.0195;
float max_motor_position[NUM_MOTORS] = {max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos, max_mot_pos};
...
float p = 400; //400; //350;//390.0;
float i_pid = 0.0;//0.1;//0.25;
float d = 0.0;//0.0;//3.75;// 0.5
...
void resetJoints(){
  ...
  for(int i = 0; i < NUM_MOTORS; i++)
  {
    desired_joint_positions[0][i] = 0.0015;
  }
...
void updateJointPositions()
{
    for(int i = 0; i < NUM_MOTORS; i++)
    {
        if(motors[i] != NULL) 
        {
            ...
            joint_positions[i] = min(max(0.02 - motor_val[i] * 0.003 * 0.02 / 5.0, 0.0),0.02);
        }
    }
}
...
//  Serial.println("correcting the positions...");
