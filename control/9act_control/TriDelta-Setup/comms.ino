...
void recvWithStartEndMarkers() {
  byte rc;
  while (Serial1.available() > 0 && newData == false) { // Serial1.available() = number of bytes (characters) available for reading from the serial port
    rc = Serial1.read(); 
...
//    Serial.println("HAKUNA");Serial.println(message.request_done_state);
    if (message.request_done_state){
      ...
      for(int i=0; i<20; i++){
        ...
        for(int j=NUM_MOTORS; j<12; j++){
          trajectory[i][j] = 0.001;
        }
      }
      for(int i=0; i < sizeof(input_cmd); i++){
        input_cmd[i] = (uint8_t)0;
...
void init_comms() {
  ...
  delay(500);
//  Serial1.println("AT+CWJAP=\"CMU-DEVICE\",\"\"");
//  delay(5000);

//  Serial1.println("AT+CWJAP=\"NETGEAR15\",\"stronghippo553\"");
//  delay(5000);
