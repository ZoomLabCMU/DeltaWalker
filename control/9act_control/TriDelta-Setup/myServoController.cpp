...
// for 100 mm motor: (4.096 / 1650) * 0.1 / 4.096 = 0.000606;
// for 20 mm motor: (4.096 / 1650) * 0.02 / 4.096 = 0.0000121212;
#define ADC2M 0.0000121212 //ADC reading to pos in [m]
// 0-255: * 15 mm/s /25 mm/a
#define CMD2VEL 0.0000920 * 15.0 / 25.0 // Motor command val to vel in [m/s]
// 1/ CMD2VEL (in theory it should be 255/ 0.025 (m/s) = 10200) 
#define VEL2CMD 10869.5652 * 25.0 / 15.0 // vel [m/s] to motor command val [x/+-255]
...
void myServoController::reset_joints() {
  ...
  float xf[NUM_MOTORS] = {0.001, 0.001, 0.001,
                          0.001, 0.001, 0.001,
                          0.001, 0.001, 0.001};
  ramp2pos(xf, 15.0/1000.0, 7.5/1000.0);
  for (int i=0; i<NUM_MOTORS; i++) {
    _motors[i]->run(RELEASE);
  }
}
